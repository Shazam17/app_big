...
class IpuRepository @Inject constructor(private val ipuDataSource: IpuDataSource) : Repository() {
    ...
    fun getByReceipt(token: String, code: String): Observable<DataResponse<List<Ipu>>> {
        return ipuDataSource.getByReceipt(token, code).compose(this.applySchedulers<DataResponse<List<Ipu>>>())
    }
}